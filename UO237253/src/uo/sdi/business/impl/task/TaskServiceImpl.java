...
public class TaskServiceImpl implements TaskService {
	...
	@Override
	public List<Task> findAllTasksByUserId(final Long id) throws BusinessException {
		return new CommandExecutor<List<Task>>().execute( new Command<List<Task>>() {
			@Override public List<Task> execute() throws BusinessException {
				
				return Persistence.getTaskDao().findAllTasksByUserId(id);
			}
		});
	}

	@Override
	public Category findCategoryByName(final String category, final Long userId)
			throws BusinessException {
		return new CommandExecutor<Category>().execute( new Command<Category>() {
			@Override public Category execute() throws BusinessException {
				
				return Persistence.getCategoryDao().findByUserIdAndName(userId, category);
			}
		});
	}
